...
db.query("INSERT INTO Message (expe,message,date,groupe) VALUES (?,?,?,?) ",[expe,message,date,groupe],(err,result)=>{
    ...
    db.query("SELECT * FROM affectation_groupe  WHERE groupe=? ",groupe,(err,result1)=>{
        ...
        if(err)
        {
            ...
            db.query("SELECT nom,prenom FROM Utilisateur WHERE id='"+expe+"'",(err,result1)=>{
 ...
 const filetype=data.filetype;
            ...
            db.query("INSERT INTO Message (expe,dest,message,date,filepath,type_file) VALUES (?,?,?,?,?,?) ",[expe,dest,message,date,filename,filetype],(err,result)=>{
                if (err){
                    console.log ("error");
                }
                else {
...
app.get('/message',(req,res)=>{
    ...
    const dest =req.query.props;
        ...
        db.query("SELECT message,date,expe,filepath,type_file FROM Message WHERE ( ( expe=? AND dest=? ) OR (dest=? AND expe=?) ) AND groupe IS NULL ",[expe,dest,expe,dest],
         (err,result2)=>{
             if (err){
                console.log(err);
                }   
...
app.get('/modifiergroupe',(req,res)=>{
    ...
    db.query("SELECT * FROM affectation_groupe INNER JOIN Groupe ON(affectation_groupe.groupe=Groupe.id) where Groupe.id='"+id+"'",(err,result)=>{
        if (err)
        {
            console.log(err);
        }
    ...
    db.query("SELECT * FROM Groupe WHERE nom_groupe=? ",nom_groupe,(err,result)=>{
       if(err)
       {
           ...
           if(result.length==0){
                ...
                db.query("INSERT INTO Groupe (nom_groupe,createdby) VALUES (?,?) ",[nom_groupe,id_connected],(err,result1)=>{
...
app.get('/message_groupe',(req,res)=>{
    const id_groupe =req.query.props;
        ...
        db.query("SELECT  concat('Groupe de discussion : ',nom_groupe) as destinataire,id,createdby FROM Groupe where id='"+id_groupe+"' ",id_groupe,(err,result)=>{
        if (err)
        {console.log(err)}
        else {

        db.query("SELECT t1.message,t1.date,t1.expe,t1.groupe,(SELECT concat(t2.nom,' ',t2.prenom) FROM Utilisateur as t2 where t1.expe = t2.id) as nom_expediteur FROM Message t1 WHERE groupe =? ",id_groupe,
         (err,result2)=>{
             if (err){
                console.log(err);
                }   
